 ...
 */
public class PrintWinningClaimTousei extends Activity implements View.OnClickListener, ReceiveListener {
    //    private String responseString;
    boolean printwinningclaimboolean;
    String saleResponse;
    Bitmap bitmap1;
    Integer pos;
    android.pt.printer.Printer printer;
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        bitmap1 = BitmapFactory.decodeResource(PrintWinningClaimTousei.this.getResources(), R.drawable.banner_tousei);
...
//        }
        ...
        if (TpsGamesClass.getInstance().getDeviceName().contains("hdx053") || TpsGamesClass.getInstance().getDeviceName().contains("TPS580") || TpsGamesClass.getInstance().getDeviceName().contains("TPS550")|| TpsGamesClass.getInstance().getDeviceName().contains("PT7003") ) {
        ...
        new Thread(new Runnable() {
            ...
            public void run() {
                try {
                    ...
                    try {
                        winningBean = TpsGamesClass.getInstance(PrintWinningClaimTousei.this).getGson().fromJson(saleResponse, WinningBean.class);
                        gameSale = TpsGamesClass.getInstance(PrintWinningClaimTousei.this).getGson().fromJson(saleResponse, GameSale.class);
                    } catch (Exception e) {
...
//                    PrintClass.printTitleSub(winningBean.getGameDevName());
//                    ThermalPrinter.printString();

                    printerClass.printTitleAtTop("SKILROCK");
                    printerClass.printSubTitle(winningBean.getGameDevName());
                    printerClass.add2String("Ticket No :", winningBean.getTicketNumber());
                    printer.printString("--------------------------------");
...
//                    PrintClass.printTwoString(PrintClass.getPrintDateFormat(winningBean.getBarcodeCount().), gameSale.getMainData().getCommonSaleData().getPurchaseTime());
                    ...
                    for (int i = 0; i < winningBean.getDrawData().length; i++) {
                        printerClass.add2String(PrintClass.getPrintDateFormat(winningBean.getDrawData()[i].getDrawDate()), winningBean.getDrawData()[i].getDrawTime());
                        ...
                        if (winningBean.getDrawData()[i].getWinStatus() != null) {
                            printerClass.add2String("WinStatus", winningBean.getDrawData()[i].getWinStatus());
                        ...
                        if (winningBean.getDrawData()[i].getWinningAmt() != null) {
                            printerClass.add2String("WinningAmt", winningBean.getDrawData()[i].getWinningAmt());
...
//
                    ...
                    String barcode = winningBean.getTicketNumber();
                    Bitmap bitmap = null;
                    ...
                    printer.setAlignment(1);
                    printer.printString(winningBean.getBarcodeCount());
                    printer.setAlignment(0);
...
//                    ThermalPrinter.walkPaper(100);
                    ...
                    printer.close();
                    isReponsePrint = true;
                    TpsGamesClass.getInstance().closeScreen(PrintWinningClaimTousei.this);
                    PrintWinningClaimTousei.this.finish();
